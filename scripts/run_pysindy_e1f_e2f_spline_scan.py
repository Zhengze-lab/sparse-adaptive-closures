#!/usr/bin/env python3
"""Run E1f/E2f: spline derivative-estimation parameter scans."""
...
RESULT_ROOTS = {
    "E1f": ROOT / "results" / "pysindy_e1f_vdp_spline_scan",
    "E2f": ROOT / "results" / "pysindy_e2f_duffing_spline_scan",
}


CONFIG = {
    "experiment_id": "E1f_E2f_spline_parameter_scan",
    "description": "High-noise spline derivative-estimation pilot for coefficient-slot SINDy.",
    "source_experiments": ["E1d_vdp_derivative_estimation_pysindy", "E2d_duffing_derivative_estimation_pysindy"],
    "noise_levels_relative_state_std": [0.05],
    "seeds": [0, 1, 2],
    "spline_s_values": [0.0, 1.0, 100.0],
...
def spline_name(value: float) -> str:
    if value == 0.0:
        return "spline_s0"
    return f"spline_s{value:.0e}".replace("+", "")


def spline_x_values() -> np.ndarray:
    values = []
    for s_value in CONFIG["spline_s_values"]:
        if float(s_value) == 0.0:
            values.append(-8.0)
        else:
            values.append(float(np.log10(float(s_value))))
...
def derivative_specs() -> list[dict[str, Any]]:
    ...
    for s_value in CONFIG["spline_s_values"]:
        specs.append(
            {
                "name": spline_name(float(s_value)),
                "family": "spline",
                "smoothing_s": float(s_value),
                "log10_s": "" if float(s_value) == 0.0 else float(np.log10(float(s_value))),
            }
        )
    return specs
...
def estimate_derivatives(
    ...
    elif spec["family"] == "spline":
        differentiator = ps.SINDyDerivative(kind="spline", s=float(spec["smoothing_s"]))
    else:
...
def aggregate(rows: list[dict[str, object]], metric_names: list[str]) -> list[dict[str, object]]:
    ...
    for (noise_level, derivative_method), group_rows in sorted(grouped.items()):
        ...
        summary = {
            ...
            "family": spec["family"],
            "smoothing_s": spec.get("smoothing_s", ""),
            "log10_s": spec.get("log10_s", ""),
...
def reference_or_best_rows(
    ...
    for noise_level, rows in sorted(by_noise.items()):
        ...
        spline_rows = [row for row in rows if str(row["family"]) == "spline"]
        for objective, metric_name in [
            ("best_spline_by_rollout", rollout_metric),
            ("best_spline_by_coefficient", "coefficient_grid_nrmse"),
            ("best_spline_by_derivative", "derivative_dx2_nrmse"),
        ]:
            row = min(spline_rows, key=lambda item: float(item[f"{metric_name}_mean"]))
...
def write_high_noise_figures(
    ...
    x_values = spline_x_values()
    spline_methods = [spline_name(float(s_value)) for s_value in CONFIG["spline_s_values"]]
    created: list[Path] = []
    for metric, y_label_en, y_label_zh, filename in [
        ...
        spline_values = np.array(
            [summary_value(summary_rows, high_noise, method, metric) for method in spline_methods],
            dtype=float,
        )
        ...
        series_en = [
            ("Spline derivative", spline_values, "#0072B2", "", "circle"),
            ("Finite difference reference", np.full_like(x_values, finite_value), "#D55E00", "8 3", "square"),
            ("Clean derivative reference", np.full_like(x_values, clean_value), "#666666", "2 3", "cross"),
        ]
        series_zh = [
            ("样条导数", spline_values, "#0072B2", "", "circle"),
            ("有限差分参考", np.full_like(x_values, finite_value), "#D55E00", "8 3", "square"),
            ("干净真导数参考", np.full_like(x_values, clean_value), "#666666", "2 3", "cross"),
        ]
        en_path = figure_dir / f"{filename}.svg"
        zh_path = figure_dir / f"{filename}_zh.svg"
        write_metric_svg(
            en_path,
            f"{system_label_en} spline scan at noise={high_noise:g}: {y_label_en}",
            x_values,
            series_en,
            "log10 spline smoothing s; s=0 plotted at -8",
            y_label_en,
        )
        write_metric_svg(
            zh_path,
            f"{system_label_zh} 样条扫描，噪声={high_noise:g}：{y_label_zh}",
            x_values,
            series_zh,
            "log10 样条平滑参数 s；s=0 画在 -8",
            y_label_zh,
        )
...
def write_outputs(
    ...
    summary_path = result_dir / "summary_by_noise_spline.csv"
    ...
    metrics = {
        ...
        "scan_config": CONFIG,
        "summary_by_noise_spline": summary_rows,
    ...
    provenance = {
        ...
        "outputs": {
            ...
            "summary_by_noise_spline_csv": str(summary_path.relative_to(ROOT)),
    ...
    hashes = {
        ...
        "summary_by_noise_spline_csv_sha256": sha256_file(summary_path),
...
def run_e1f() -> dict[str, object]:
    result_dir = RESULT_ROOTS["E1f"]
                ...
                seed_rows.append(
                    {
                        ...
                        "smoothing_s": deriv_spec.get("smoothing_s", ""),
                        "log10_s": deriv_spec.get("log10_s", ""),
    ...
    best_rows = reference_or_best_rows("E1f_vdp", summary_rows, "interp_rollout_nrmse")
    ...
    write_outputs(
        ...
        figure_paths,
        {
            "dataset_id": "pysindy_e1f_vdp_spline_scan",
            "experiment_id": "E1f_vdp_spline_scan",
            ...
            "purpose": "Tune spline derivative-estimation smoothing for Van der Pol coefficient-slot discovery.",
...
def run_e2f() -> dict[str, object]:
    result_dir = RESULT_ROOTS["E2f"]
    ...
    summary_rows = aggregate(seed_rows, metric_names)
    best_rows = reference_or_best_rows("E2f_duffing", summary_rows, "test_rollout_nrmse")
    figure_paths = write_high_noise_figures(
        "E2f Duffing",
        "E2f Duffing",
        summary_rows,
    ...
    write_outputs(
        ...
        figure_paths,
        {
            "dataset_id": "pysindy_e2f_duffing_spline_scan",
            "experiment_id": "E2f_duffing_spline_scan",
            ...
            "purpose": "Tune spline derivative-estimation smoothing for Duffing stiffness-slot discovery.",
        ...
        json.dumps(
            {
                ...
                "seeds": CONFIG["seeds"],
                "spline_s_values": CONFIG["spline_s_values"],
                "optimizer": CONFIG["optimizer"],
                "results": {"E1f": e1_result["result_dir"], "E2f": e2_result["result_dir"]},
            },
            indent=2,
            ensure_ascii=False,
        )
